...
class PollInItemEditDialog(QDialog):
    """
    ...
    def _update_visibility(self):
        ...
        # 1. type이 bitmap일 때는 ui_type을 table로 강제
        if current_type == "bitmap":
            self.ui_type_combo.setCurrentText("table")
            current_ui_type = "table"
            
        # 2. type이 bitmap이 아닌데 ui_type이 table로 되어있다면
        elif current_type != "bitmap" and current_ui_type == "table":
            # 요청하신 대로 type을 uint8로 변경 (모순 방지를 위해 ui_type도 기본값인 number로 변경)
            self.type_combo.setCurrentText("uint8")
            self.ui_type_combo.setCurrentText("number") # 필요시 "enum"으로 변경 가능
            current_type = "uint8"
